...
{
    return gv_listen_api_msg(sd->so_ctrl, (u_int8_t)backlog);
}
...
{
    int tmp_sd;
    if (gv_connect_api_msg(sd->so_ctrl,
			   addr.sin_addr.in_addr,
			   addr.sin_port,
			   addr.sin_gvport,
			   sd->so_addr.sun_path) != -1)
    {
	while(1)
	{
	    tmp_sd = accept(sd->so_data, 0, 0);
	    if (tmp_sd == -1)
	    {
		if(errno == EINTR)
		    continue;
...
{
    return gv_bind_api_msg(sd->so_ctrl, 
			   &(addr->sin_addr.in_addr),
			   &(addr->sin_port),
			   &(addr->sin_gvport));
}
...
{
    int tmp_sd;
    if (gv_accept_api_msg(sd->so_ctrl,
			  &(addr.sin_addr.in_addr),
			  &(addr.sin_port),
			  &(addr.sin_gvport),
			   sd->so_addr.sun_path) != -1)
